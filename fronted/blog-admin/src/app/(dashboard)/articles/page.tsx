...
export default function ArticlesPage() {
  ...
  return (
    <div className="space-y-5">
        ...
        <table className="w-full text-xs text-left border-collapse min-w-[900px] table-fixed">
          <thead>
            <tr className="border-b border-zinc-150 dark:border-zinc-850 bg-zinc-50/70 dark:bg-zinc-900/40 text-[10px] font-bold text-zinc-400 dark:text-zinc-500 uppercase tracking-widest select-none">
              ...
              <th className="px-5 py-3.5 w-[280px]">文章标题</th>
              <th className="px-5 py-3.5 w-28">所属分类</th>
              <th className="px-5 py-3.5 w-24 text-center">访问权限</th>
              ...
              <th className="px-5 py-3.5 w-32">更新日期</th>
              ...
              articles.map((article) => {
                ...
                return (
                  <tr 
                    key={article.id} 
                    className={cn(
                      "hover:bg-zinc-50/50 dark:hover:bg-zinc-900/15 transition-all group",
                  ...
                  >
                    ...
                    {/* 标题（包含链接跳转、火苗标识） */}
                    <td className="px-5 py-3.5 w-[280px]">
                      <div className="flex items-center gap-1.5 max-w-full">
                        <div 
                          className="font-semibold text-neutral-dark dark:text-zinc-150 truncate max-w-[200px] leading-tight" 
                          title={article.title}
                        >
                          {article.title}
                        </div>
                      ...
                      {article.summary && (
                        <div 
                          className="text-[11px] text-zinc-450 dark:text-zinc-500 truncate max-w-[240px] mt-1 font-normal leading-normal" 
                    ...
                    <td className="px-5 py-3.5 w-28">
                      {article.categoryName ? (
                        <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-zinc-100/60 dark:bg-zinc-900/60 border border-zinc-200/40 dark:border-zinc-800/60 text-zinc-500 dark:text-zinc-400">
                          {article.categoryName}
                        </span>
                      ) : (
                    ...
                    {/* 更新日期 */}
                    <td className="px-5 py-3.5 w-32 text-zinc-450 dark:text-zinc-500 font-mono text-[10px] leading-tight">
                      {article.updatedAt
      ...
      <div 
        className={cn(
          "fixed bottom-6 left-1/2 -translate-x-1/2 z-35 flex items-center gap-4.5 px-6 py-3 rounded-2xl border bg-white/95 dark:bg-zinc-900/95 backdrop-blur-md shadow-[0_15px_30px_rgba(0,0,0,0.12)] border-zinc-200/60 dark:border-zinc-800/60 transition-all duration-300 ease-out select-none",
          checkedIds.length > 0 
            ? "opacity-100 translate-y-0 scale-100 pointer-events-auto animate-[slideUp_0.25s_ease-out]" 
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="h-8 px-3.5 rounded-xl bg-zinc-100 dark:bg-zinc-800 text-[10px] font-bold text-zinc-650 dark:text-zinc-350 hover:bg-zinc-200/80 dark:hover:bg-zinc-750 transition-all cursor-pointer flex items-center gap-1.5 disabled:opacity-50 disabled:cursor-not-allowed border-0 outline-none active:scale-95"
          >
            <span>设为草稿</span>
          </button>
