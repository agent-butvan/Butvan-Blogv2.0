...
export default function ArticleForm({ initialData, onSave, saving = false }: ArticleFormProps) {
  ...
  const inputClass =
    "w-full rounded-lg border border-zinc-200 bg-white px-4 py-3 text-sm text-zinc-900 placeholder-zinc-400 transition-colors duration-150 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20";
      ...
      {activeTab === "content" && (
        <div className="space-y-5">
          ...
          <input
            ...
            placeholder="请输入文章标题"
            required
            className="w-full rounded-lg border-0 bg-transparent px-0 text-3xl font-bold font-heading text-neutral-dark placeholder-zinc-300 outline-none focus:ring-0"
          />
      ...
      {activeTab === "settings" && (
        <div className="space-y-5 max-w-2xl">
          ...
          <div>
            ...
            {coverImageUrl && (
              <img
                src={coverImageUrl}
                alt="封面预览"
                className="mt-2 h-40 w-full rounded-lg object-cover border border-zinc-200"
              />
            )}
          </div>

          {/* SEO 设置 */}
          <fieldset className="rounded-xl border border-zinc-200 p-5">
            <legend className="px-2 text-sm font-medium text-zinc-600">SEO 元数据</legend>
            <div className="space-y-4">
              <div>
                <label className="mb-1 block text-xs text-zinc-400">SEO 标题</label>
                <input
                  type="text"
                  value={seoTitle}
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-400">SEO 描述</label>
                <textarea
              ...
              <div>
                <label className="mb-1 block text-xs text-zinc-400">SEO 关键词（逗号分隔）</label>
                <input
                  type="text"
                  value={seoKeywords}
          ...
          <div className="flex items-center gap-3 rounded-lg bg-zinc-50 p-4">
            ...
            <span className="text-sm text-zinc-500">目前默认为公开可见，私密/密码保护功能即将上线</span>
          </div>
        </div>
      )}

      {/* 底部操作栏 */}
      <div className="flex items-center justify-between border-t border-zinc-200 pt-5 sticky bottom-0 bg-neutral-light py-4">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg border border-zinc-300 bg-white px-5 py-2.5 text-sm font-medium text-zinc-700 hover:bg-zinc-50 transition-colors disabled:opacity-50"
          >
            <Save size={16} /> 保存草稿
          </button>
          <button
