...
export default function HotspotPropertiesPanel({
  ...
  useEffect(() => {
    const redirectType = hotspot?.redirectType;
    if (!redirectType) return;

    const timer = setTimeout(() => {
      // 按需懒加载：仅首次切换到对应类型时请求
      if (redirectType === 'ARTICLE' && articles.length === 0 && !articlesLoading) {
      ...
      if (redirectType === 'CATEGORY' && categories.length === 0 && !categoriesLoading) {
      ...
      {!hotspot ? (
        ...
        <div className="flex flex-col gap-4 text-left">
          ...
          <div className="flex flex-col gap-2">
            <label className="text-xs font-heading font-medium text-zinc-650 dark:text-zinc-350">
              物品图预览
            </label>
              ...
              <div className="relative aspect-video rounded-xl overflow-hidden border border-zinc-200 dark:border-zinc-800 bg-zinc-50 dark:bg-zinc-950/30">
                ...
                <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 flex flex-col items-center justify-center gap-2 transition-opacity">
                  ...
                  <button
                    ...
                    disabled={uploading}
                    className="flex items-center gap-1 px-2.5 py-1.5 rounded-md bg-white dark:bg-zinc-800 text-zinc-900 dark:text-zinc-100 text-xs font-heading hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors duration-200 cursor-pointer shadow-sm"
                  >
            ...
            <span className="text-[10px] text-zinc-550 dark:text-zinc-400 leading-relaxed font-sans">
          ...
          <div className="flex flex-col gap-1">
            ...
            <input
              ...
              className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 h-8 text-xs text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 dark:placeholder-zinc-400 focus:outline-none focus:ring-1.5 focus:ring-primary/20 focus:border-primary transition-all duration-200"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(
              ...
              <div className="flex flex-col gap-1" key={key}>
                <label className="text-[10px] text-zinc-650 dark:text-zinc-350 font-mono">
                  {label}
                </label>
                <input
                  value={
                  ...
                  className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-2 h-7 text-[11px] text-zinc-900 dark:text-zinc-100 text-center font-mono focus:outline-none focus:ring-1.5 focus:ring-primary/20 focus:border-primary transition-all duration-200"
                />
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex flex-col gap-1">
              <label className="text-[10px] font-heading font-medium text-zinc-650 dark:text-zinc-350">
              ...
              {hotspot.redirectType === 'INTERNAL' && !isCustomPath ? (
                ...
                <div className="flex gap-1">
                  <input
                    ...
                    className="flex-1 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 h-8 text-xs text-zinc-900 dark:text-zinc-100 placeholder-zinc-500 dark:placeholder-zinc-400 focus:outline-none focus:ring-1.5 focus:ring-primary/20 focus:border-primary transition-all duration-200"
                  />
                  <button
                    type="button"
                    onClick={() => setIsCustomPath(false)}
                    className="px-2 h-8 rounded-lg border border-zinc-200 dark:border-zinc-700 text-zinc-650 dark:text-zinc-350 text-[10px] hover:bg-zinc-100 dark:hover:bg-zinc-800 transition-all duration-200 cursor-pointer shrink-0"
                    title="返回列表选择"
                  >
                    📋
                  </button>
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input
                ...
                className="w-full bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-3 h-8 text-xs text-zinc-900 dark:text-zinc-100 font-mono placeholder-zinc-500 dark:placeholder-zinc-400 focus:outline-none focus:ring-1.5 focus:ring-primary/20 focus:border-primary transition-all duration-200"
              />
            </div>
          </div>
          ...
          <div className="flex items-center justify-between border-t border-zinc-200 dark:border-zinc-800 pt-3">
            <span className="text-xs font-heading font-medium text-zinc-650 dark:text-zinc-350">
              前台显示
            </span>
            <select
              ...
              className="text-[11px] bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 rounded-lg px-2.5 py-1 text-zinc-800 dark:text-zinc-200 focus:outline-none focus:ring-1.5 focus:ring-primary/20 focus:border-primary w-20 h-7 cursor-pointer transition-all duration-200"
            >
          ...
          <button
            onClick={onSave}
            disabled={saving}
            className="flex items-center justify-center gap-1.5 w-full py-2.5 rounded-lg bg-primary text-white text-sm font-heading font-medium hover:opacity-90 hover:shadow-md transition-all duration-200 disabled:opacity-50 active:scale-98 cursor-pointer"
          >
            {saving ? (
              <>
