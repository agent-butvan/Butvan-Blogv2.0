                ...
                <div className="animate-detail-item opacity-0 mt-10 pt-8 border-t border-zinc-200/50 dark:border-zinc-800/60 select-none">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {article.relatedArticles.slice(0, 2).map((rel) => {
                      // 仅当文章介质类型为手记 (NOTE) 时才展示封面图，普通文章默认隐藏封面
                      const showCover = rel.contentType === 'NOTE' && !!rel.coverImageUrl

                      return (
                        <Link
                          key={rel.id}
                          href={`/article/${rel.slug}`}
                          className="group relative flex flex-col justify-between p-4 rounded-none border border-zinc-200/80 dark:border-zinc-800 bg-white/50 dark:bg-zinc-900/30 hover:bg-white dark:hover:bg-zinc-900 hover:border-primary/50 dark:hover:border-primary/50 transition-all duration-300 hover:shadow-sm overflow-hidden"
                        >
                          <div className="flex items-start gap-3">
                            {showCover && (
                              <div className="w-16 h-16 rounded-none overflow-hidden shrink-0 border border-zinc-200/60 dark:border-zinc-800 bg-zinc-100 dark:bg-zinc-800">
                                <img
                            ...
                            <div className="flex-1 min-w-0">
                              {rel.categoryName && (
                                <span className="inline-block px-1.5 py-0.5 text-[9px] font-medium text-primary bg-primary/10 rounded-none mb-1">
                                  {rel.categoryName}
                                </span>
                              )}
