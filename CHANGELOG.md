...
本次更新重磅支持了编辑器 HTML 嵌入区域保存与自定义标题、后台个人中心赞赏/收款二维码图片上传与前台赞赏 Modal 动态展示、文章详情页延伸阅读关联推荐与智能补全推荐算法，以及全站直角边框与细节体验优化。

### 功能新增与体验重构

- **文章编辑器 HTML 嵌入与自定义标题**：解决 Tiptap Markdown 序列化丢失 iframe 问题，支持显示与动态编辑 HTML 嵌入区域的独立自定义标题。
- **个人中心赞赏收款码配置**：后台个人中心支持选择本地图片上传赞赏码或手动输入 URL 地址；前台赞赏 Modal 自动实时渲染该收款码。
- **文章延伸阅读关联推荐**：支持作者在后台文章编辑侧边抽屉中选择最多 2 篇关联推荐文章；若未设置或不足 2 篇，系统自动基于同分类/时间权重补全，100% 保证前台底部恒定精致展示 2 篇推荐文章。
- **全站 UI 细节点缀与直角化风格**：重构前台评论区已登录邮箱区域为直角边框，调整延伸阅读卡片为直角边框并严格限定仅手记（`NOTE`）类型文章展示封面图。

### Commits

- style(ui): 调整文章详情页延伸阅读区域样式为直角边框且仅手记类型展示封面图
- feat(article): 新增文章详情页延伸阅读关联推荐功能与后台手动配置面板
- fix(user): 修复上传或修改赞赏收款码后保存资料按钮未激活的问题
- feat(user): 支持在个人中心配置赞赏收款码并在前台赞赏弹窗动态渲染
- style(ui): 调整评论区已登录邮箱区域样式为无阴影无背景色直角边框
- style(ui): 使用Portal重构标题弹窗实现整屏侧边栏背景虚化并对齐系统Primary主题色
- refactor(editor): 替换原生prompt为UI弹窗并移除后台HTML嵌入区域全屏效果
- feat(editor): 支持自定义设置与在线编辑HTML页面预览区域标题
- style(ui): 调整HTML嵌入预览区样式移除底部阴影并添加预览标识边框
- fix(admin): 修复文章编辑器上传HTML页面保存并刷新后内容丢失的问题
